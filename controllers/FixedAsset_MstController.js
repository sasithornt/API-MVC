...
async function getFixedAssetBy(Id) {
    try {
        let pool = await sql.connect(config);
        let c = await pool.request().input('id_parameter', sql.Int, Id)
            .query("SELECT FA.Id, FA.Asset_Code , CG.Id [Category] ,"+
            " TP.Id [Types],FA.Name ,SM.Id [Status_Mst],FA.Regis_Date,FA.Begin_Use,FA.DestroyRequest_Date,FA.Destroy_Date,FA.PurchaseAmt [Price],FA.Qty,UM.Id [Um] ," +
            " FA.Amount, LO.Id [Location], US.Id [USERID] ," +
            " MD.Id [Model],MF.Id [Manufacture],FA.Serail ,FA.Useful,FA.AccruDrep,FA.Amorlization,FA.CurrentDrep "+
            " FROM	FIXEDASSET_MST FA  " + 
            " LEFT JOIN LOCATION LO	ON LO.ID = FA.LOCATION_ID " +
            " LEFT JOIN UM			ON UM.ID = FA.UM_ID " +
            " LEFT JOIN CATEGORY CG	ON CG.ID = FA.CATEGORY_ID " +
...
module.exports = {
    ...
    getFixedAssetBy : getFixedAssetBy,
